...
export const TRACK_KEY_PREFIX = STORAGE.TRACK_KEY_PREFIX;
...
export function migrateTrackHistory() {
  ...
  return local
    .get(STORAGE.LEGACY_TRACK_HISTORY_KEY)
    .then((items) => {
      migrationDone = true;
      const blob = items[STORAGE.LEGACY_TRACK_HISTORY_KEY];
      ...
      if (Object.keys(toSet).length === 0) {
        return local.remove(STORAGE.LEGACY_TRACK_HISTORY_KEY);
      }
      return local.set(toSet).then(() => local.remove(STORAGE.LEGACY_TRACK_HISTORY_KEY));
    })
    .catch(() => {
      migrationDone = true;
    });
