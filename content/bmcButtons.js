...
PD.injectBmcButtons = function () {
  ...
  if (type === 'MusicRecording') {
    ...
    if (trackID.includes('-')) {
      scbuttonurl = 'https://soundcloud.com/search?q=' + token(trackID);
      trackURL = 'https://www.buymusic.club/search/' + token(trackID);
    } else {
      scbuttonurl = 'https://soundcloud.com/search?q=' + token(artistID + ' ' + trackID);
      trackURL = 'https://www.buymusic.club/search/' + token(artistID + ' ' + trackID);
    }
    albumURL = 'https://www.buymusic.club/search/' + token(artistID + ' ' + albumID);
    artistURL = 'https://www.buymusic.club/search/' + token(artistID);
    ...
    const artistID = PD.data.byArtist.name;
    scbuttonurl = 'https://soundcloud.com/search?q=' + token(artistID + ' ' + albumID);
    albumURL = 'https://www.buymusic.club/search/' + token(artistID + ' ' + albumID);
    artistURL = 'https://www.buymusic.club/search/' + token(artistID);
    scbutton.onclick = () => window.open(scbuttonurl, '_blank');
    ip.append(scbutton);
  ...
  function token(query) {
    return globalThis.pdUrlBuilder.createSearchToken(query);
  }

  async function validateOne(entry) {
    const valid = await PD.api.bmcValidate(entry.url);
